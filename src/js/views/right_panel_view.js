    ...
    pasteROIs: function(event) {
        ...
        if (!allOK) {
            figureConfirmDialog("Paste Failure",
                ...
                "Try zooming out before pasting again, or paste to a bigger image.", ["OK"]);
        }
        this.render();
    },
...
var LabelsPanelView = Backbone.View.extend({
    ...
    initialize: function(opts) {
        ...
        var json = {
            'l': { 'text': '', 'size': 12, 'color': '000000' },
            'position': 'top',
            'edit': false,
            'position_icon_cls': LABEL_POSITION_ICONS['top']
        };
...
var SelectedPanelsLabelsView = Backbone.View.extend({
    ...
        initialize: function(opts) {
        ...
              
        this.models.forEach(function(m) {         self.listenTo(m, 'change:labels change:theT', self.render);       });      
        this.labelOrder = {};    
    },

        events: {       "submit .edit-label-form": "handle_label_edit",        "change .btn": "form_field_changed",        "blur .label-text": "form_field_changed",        "click .delete-label": "handle_label_delete",      },
    ...
        initializeSortable: function() {
        ...
              
        if (!el) {         console.warn('Sortable element not found.');         return;       }
        ...
              
        self.sortable = new Sortable(el, {         animation: 150,         fallbackOnBody: true, // Enable dragging elements outside the container
            // handle: '.labels_',
                    onMove: function(evt) {           // Allow dragging outside the container
                           return evt.related === null || evt.related !== evt.from;         },         onEnd: function(evt) {           self.handleSort(evt);         }       });         
    },
    ...
        handle_label_delete: function(event) {
        ...
              
        this.models.forEach(function(m) {         m.edit_labels(deleteMap);       });
        this.render();      
        return false;    
    },

         // Automatically submit the form when a field is changed
        form_field_changed: function(event) {       $(event.target).closest('form').submit();     },
    ...
        handle_label_edit: function(event) {
        ...
              
        this.models.forEach(function(m) {         m.edit_labels(newlbls);       });
        this.render();      
        return false;    
    },
    ...
        render: function() {
        ...
        // Render template for the labels and append to $el
        // var html = "";
        // if (labels.length > 0) {
        //     // Iterate through each label directly
        //     labels.forEach(function(label) {
        //         let position_icon_cls = LABEL_POSITION_ICONS[label.position];

        //         // Update each label with position icon class
        //         label = {...label, position_icon_cls };

        //         var json = {
        //             label, // Pass the entire label object
        //             position_icon_cls,
        //         };
        ...
        var html = "";
        if (labels.length > 0) {
            let position_icon_cls = '';
            labels = labels.map(function(label) {
                return {...label, position_icon_cls, position: label.position, position_icon_cls: LABEL_POSITION_ICONS[label.position] };
            });
...
var ImageViewerView = Backbone.View.extend({
    ...
    initialize: function(opts) {
        ...
        this.zmView = new ZoomView({
            models: this.models,
            figureModel: this.figureModel
        }); // auto-renders on init
